        ...
        <template #activator="{ props: menu }">
            <VTooltip :text="translate('user')" location="bottom">
                <template #activator="{ props: tooltip }">
                    <VAvatar v-bind="{ ...menu, ...tooltip }" :image="logo" size="24" />
                </template>
            </VTooltip>
        </template>
        <VList>
            <VListItem :prepend-avatar="logo" :title="user?.email" :subtitle="translate('admin')"></VListItem>
            <VDivider class="my-2" />
            ...
            <template v-for="(item, index) in currentItems" :key="index">
                ...
                <VListItem :value="item?.title" :prepend-icon="item?.icon" @click="onClickMenuItem(item)">
                    <VListItemTitle>{{ translate(item?.title) }}</VListItemTitle>
                </VListItem>
            </template>
        </VList>
    </VMenu>
