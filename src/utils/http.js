...
export const GET = async (path, options = {}) => {
    try {
        ...
        return { error: null, data: data, totalCount: Number(res.headers['x-total-count']) } // json-server header for dev purpose
    } catch (error) {
...
axios.interceptors.response.use(
    ...
    async function (error) {
        ...
        // temporary checking
        if (error.response.status === 401 && error.response?.data.includes('jwt')) {
            store.logout()
        }
